 ...
 * and tweaked for use in the logger theme.
 *
...
add_action( 'after_switch_theme', 'logger_switch_theme' );
add_action( 'load-customize.php', 'logger_load_customize' );
add_action( 'template_redirect', 'logger_preview' );
 ...
 * @return string
 */
function logger_compat_message()
{
    if ( version_compare( $GLOBALS['wp_version'], '5.0', '<' ) ) {
        return sprintf(
            /* Translators: 1 is the required WordPress version and 2 is the user's current version. */
            esc_html__( 'Logger requires at least WordPress version %1$s. You are running version %2$s. Please upgrade and try again.', 'logger' ),
            '5.2',
            $GLOBALS['wp_version']
        );
    }
    ...
    if ( version_compare( PHP_VERSION, '7.0', '<' ) ) {
        return sprintf(
            ...
            esc_html__( 'Logger requires at least PHP version %1$s. You are running version %2$s. Please upgrade and try again.', 'logger' ),
            '7.2',
            PHP_VERSION
        );
    }
 ...
 * @return void
 */
function logger_upgrade_notice()
{

    printf( '<div class="error"><p>%s</p></div>', esc_html( logger_compat_message() ) );
}
 ...
 * @return void
 */
function logger_load_customize()
{
    wp_die( esc_html( logger_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function logger_preview()
{
    if ( isset( $_GET['preview'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
        wp_die( esc_html( logger_compat_message() ) );
    }
}
